...
export default function CourseWorkCard({ work }: { work: StudentWorkRow }) {
  const detailHref = work.slug ? `/works/students/${work.slug}` : null;
  const returnTo =
    work.course_id && work.academic_year
  ...
  const pdfHref = buildPdfViewerHref({
    ...
    returnTo,
    returnLabel: "กลับไปผลงานรายวิชา",
    source: "course",
  });
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm ring-1 ring-slate-100/70 transition hover:-translate-y-0.5 hover:border-brand-200 hover:shadow-lg">
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        {work.course_name && <p className="mb-2 text-xs font-medium text-slate-500">{work.course_name}</p>}
        ...
        {detailHref ? (
          ...
          <h2 className="break-words text-[15px] font-semibold leading-relaxed text-slate-900">{work.title}</h2>
        )}
        ...
        <div className="mt-3 space-y-1.5 text-xs text-slate-500">
          ...
          {work.advisor_name && (
            <div className="flex gap-2">
              ...
              <span className="line-clamp-1">อาจารย์ผู้สอน: {work.advisor_name}</span>
            </div>
          )}
        </div>
